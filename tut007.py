...
class ContentSummarizer:
    """Advanced content summarization with keyword extraction and tagging"""
    ...
    def __init__(self, logger=None):
        ...
        try:
            ...
            self.logger.log(f"⚠️ Using basic stopwords: {str(e)[:50]}", "warning")
            self.stop_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'being', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'can', 'this', 'that', 'these', 'those'}
        ...
        try:
            ...
            words = [
                ...
                if (word.isalnum() and 
                    ...
                    len(word) > 2 and 
                    len(word) < 20 and  # Avoid very long words
                    not word.isdigit())  # Avoid pure numbers
            ]
            ...
            # Filter to meaningful keywords (length > 3 preferred)
            ...
            return good_keywords[:num_keywords]
            
        except Exception as e:
            self.logger.log(f"❌ Error extracting keywords: {str(e)[:100]}", "error")
            # Return basic keywords as fallback
            basic_words = re.findall(r'\b[a-z]{4,}\b', text.lower())
            return list(set(basic_words))[:num_keywords] if basic_words else []
    ...
    def generate_tags(self, text: str, keywords: List[str], num_tags: int = 8) -> List[str]:
        ...
        try:
            ...
            # Use keywords as base for tags
            base_tags = keywords[:num_tags//2]
            
            # Extract key phrases (simple n-grams)
            try:
                sentences = sent_tokenize(text[:2000])  # Limit for efficiency
            except Exception:
                # Fallback to simple sentence splitting
                sentences = [s.strip() for s in text[:2000].split('.') if len(s.strip()) > 10]
            
            phrases = []
            
            for sentence in sentences[:15]:  # Limit sentences
                # Clean sentence
                ...
                for i in range(len(words) - 1):
                    if i < len(words) - 1:
                        ...
                        if len(phrase) > 5 and len(phrase) < 30:  # Reasonable length
                            phrases.append(phrase)
                
                for i in range(len(words) - 2):
                    if i < len(words) - 2:
                        phrase = f"{words[i]} {words[i+1]} {words[i+2]}"
                        if len(phrase) > 8 and len(phrase) < 40:  # Reasonable length
                            phrases.append(phrase)
            ...
            for tag in base_tags + phrase_tags:
            ...
            # If we don't have enough tags, add some from keywords
            if len(all_tags) < num_tags:
                for kw in keywords:
    ...
    def create_two_liner(self, text: str) -> str:
        ...
        try:
            ...
            try:
                sentences = sent_tokenize(text[:2000])  # Limit text for efficiency
            except Exception:
            ...
            for sent in sentences:
                sent = sent.strip()
                if len(sent) > 15 and len(sent) < 200:  # Reasonable sentence length
                    clean_sentences.append(sent)
            
            if len(clean_sentences) >= 2:
                # Use first two meaningful sentences
                line1 = clean_sentences[0][:120]  # Limit length
                line2 = clean_sentences[1][:120] if len(clean_sentences) > 1 else "Additional insights and key information provided."
                
                # Ensure they end properly
                ...
                # Split single sentence or create second line
                line1 = clean_sentences[0][:120]
                if not line1.endswith(('.', '!', '?')):
                    line1 += '.'
                line2 = "Contains detailed information and comprehensive analysis."
                return f"{line1}\n{line2}"
            else:
                # Fallback for very short or problematic content
                return f"{text[:100].strip()}.\nProvides essential information and key insights."
    ...
    def generate_detailed_summary(self, text: str, source_type: str) -> str:
        ...
        try:
            ...
            try:
                sentences = sent_tokenize(text[:5000])  # Increased limit
            except Exception:
                # Fallback sentence splitting
                sentences = [s.strip() for s in text[:5000].split('.') if len(s.strip()) > 10]
            
            if len(sentences) <= 3:
                return " ".join(sentences)
            
            # Extract keywords for importance scoring
            ...
            for i, sentence in enumerate(sentences):
                ...
                keyword_count = sum(1 for kw in keyword_set if kw in sentence_lower)
            ...
            selected_sentences = sentence_scores[:6]  # Select top 6 sentences
            ...
            if len(summary) < 100:
                ...
                additional_sentences = [sent[0] for sent in sentence_scores[6:10]]
                summary = " ".join(summary_sentences + additional_sentences)
            
            # Limit maximum length
            if len(summary) > 1000:
                summary = summary[:1000] + "..."
            
            return summary if len(summary) > 50 else "This content provides comprehensive information on the topic with detailed explanations, key concepts, and valuable insights for learning and understanding."
    ...
    def generate_summary(self, content_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                self.logger.log(f"✅ Extracted {len(keywords)} keywords", "success")
                
                # Step 2: Generate tags
                status_text.text("🏷️ Generating descriptive tags...")
                progress_bar.progress(50)
                tags = self.generate_tags(text, keywords, 10)
                self.logger.log(f"✅ Generated {len(tags)} tags", "success")
...
with tab3:
    ...
    with col1:
        ...
        already_summarized = any(
            summary.get('original_title') == selected_content['title'] or 
...
with tab2:
    ...
    content_type = st.radio(
        "Select content type:",
        ["🌐 Website", "📄 PDF", "🎥 YouTube", "🎵 Audio", "📝 Text"],
        horizontal=True
    )
...
with tab1:
    if "content_loaded" not in st.session_state:
        ...
        2. Choose your content type (Website, PDF, YouTube, Audio, Text)
...
with tab5:
    ...
    with col1:
        filter_type = st.selectbox(
            "Filter by type:",
            ["All", "web", "pdf", "youtube", "audio", "text", "summary"]
        )
    with col2:
